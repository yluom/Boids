...
/**
 * Bird --- class to represent an bird-like object in simulation of the flocking
 * behaviour of birds.
 * 
 * @author fr05td3su
 */
class Bird {
	...
	/**
	 * Calculate new velocity vector based on current velocity, cohesion, alignment
	 * and separation coefficients and bound position.
	 *
	 * @param birds List of birds, which positions and velocities are used to
	 *              calculate corresponding vectors of cohesion, alignment,
	 *              separation xMax Maximum value of x-coordinate of position yMax
	 *              Maximum value of y-coordinate of position cohesionCoefficient
	 *              Value affects speed at which bird moves towards the perceived
	 *              centre of mass e.g 100 means that in each iteration bird moves
	 *              1% to the perceived centre alignmentCoefficient Value affects
	 *              velocity increase of bird with respect to the perceived centre
	 *              of mass separationCoefficient If bird is within this distance
	 *              from other birds, it will move away
	 * @return No return value.
	 */
	public void updateVelocity(Bird[] birds, int xMax, int yMax, double cohesionCoefficient, int alignmentCoefficient,
			double separationCoefficient) {
		velocity = velocity.plus(cohesion(birds, cohesionCoefficient)).plus(alignment(birds, alignmentCoefficient))
				.plus(separation(birds, separationCoefficient)).plus(boundPosition(xMax, yMax));
		limitVelocity();
	}
